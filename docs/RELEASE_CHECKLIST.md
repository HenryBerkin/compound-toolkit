...
| Project foundation — iOS Engineer / QA | Design storage in Application Support with data protection, atomic writes, recovery copy, safe unknown-schema handling, delete-one/reset-all. | **Release-time verification** | **Partial pass under IGC-012/IGC-D021:** scenario-store Application Support, data-protection request, atomic replacement, exact-source recovery, unknown-schema handling and delete-one are accepted with 40/40 unit/fixture/store and 16/16 UI evidence. Settings-wide reset-all is authorised in IGC-013 but not implemented; supported-device release recheck remains open. No absolute encryption claim. |
| Feature-complete — PM / Designer | Ensure calculator/results/education communicate assumptions, hypothetical nature and limitations; remove personal recommendation, guarantee, regulated-service, account/live-data/payment implications. | **Legal/regulatory review recommended** | IGC-013 is authorised to implement restrained development/internal-beta content. Final external-beta/public copy approval and UI screenshots remain **Open**. |
| Feature-complete — iOS Engineer | Verify offline calculation/scenario lifecycle; no network calls, accounts, permissions, tracking, analytics, support diagnostics or exports/sharing. | **Release-time verification** | **Partial pass under IGC-007/IGC-012:** simulator/offline proxy and static dependency/API/config evidence confirm the implemented calculator and scenario lifecycle are local-only with no listed excluded surface. Release-candidate network/archive and supported-device recheck remain open. |
| Feature-complete — QA / Designer | Validate Dynamic Type, VoiceOver, contrast, dark mode, Reduce Motion, keyboard/focus, legible validation, text alternative for chart, iPhone and adaptive iPad layouts. | **Release-time verification** | **Partial:** IGC-007 automated/simulator evidence and requested physical smoke passed. IGC-013 owns secondary-content/preference coverage; complete feature and release-candidate matrix remains open. |
| Feature-complete — iOS Engineer / App Store Reviewer | Inventory all APIs and dependencies; author/validate actual privacy manifest only when justified; generate privacy report and match it to questionnaire answers. | **Release-time verification** | IGC-D022 requires IGC-013’s app target to declare app-only UserDefaults reason `CA92.1`; implementation placement/validation and later archive privacy report remain **Open**. |
