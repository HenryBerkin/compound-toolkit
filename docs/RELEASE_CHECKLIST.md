...
| Before TestFlight — Product Owner / App Store Reviewer | Verify release archive built with currently required Xcode/SDK; inspect architectures, target/platforms, signed entitlements, capabilities, icon asset processing and binary contents. | **Release-time verification** | **Passed and uploaded 2026-07-30:** fresh archive `/private/tmp/IGC-1.0-1-TestFlight.xcarchive` was built with Xcode 26.6 / iOS 26.5 SDK and inspected before upload. It is arm64 iOS, iOS 17+, iPhone+iPad, Team `2FKVFS8X67`, validly signed, and contains no extension/framework. The compiled bundle contains phone/pad `AppIcon` files and primary-icon metadata. App Store Connect processed build `1.0 (1)` as Binary State **Validated**, with the correct bundle ID, device family, architecture, no hidden icon and no unintended entitlement. |
| Before TestFlight — App Store Reviewer | Prepare beta description, test focus and feedback email; answer export-compliance questions from actual cryptography evidence. | **Release-time verification** | **Passed for internal TestFlight 2026-07-30:** App Store Connect has the English (U.K.) beta description, `support@mochadesigns.co.uk` feedback email and build-specific test focus. From the accepted first-party offline binary inventory, the current Apple questionnaire was answered **None of the algorithms mentioned above**; processed metadata records **App Uses Non-Exempt Encryption: No**. Revisit after any cryptography/network/SDK change. |
| Before TestFlight — PM / QA | Make a privacy policy/support page live, verify URLs/contact details and policy/UI alignment; complete provisional App Privacy decision review. | **Release-time verification** | **Public URL and internal-TestFlight metadata gates passed 2026-07-30:** `https://igc.mochadesigns.co.uk/privacy` and `https://igc.mochadesigns.co.uk/support` are owner-controlled Cloudflare Pages routes; the URLs are saved in App Store Connect, and TestFlight also records the PWA root as Marketing URL and the Privacy URL. Support identifies `support@mochadesigns.co.uk`. No analytics, tracking, Function, database or paid service is enabled. Final App Privacy questionnaire entry remains a public-submission operation. |
| Internal TestFlight — Product Owner / PM | Upload the validated build, create an internal group, attach the build, invite the owner and retain the 90-day expiry boundary. | **Release-time verification** | **Passed 2026-07-30:** Xcode upload succeeded; App Store Connect processed build UUID `4c40e517-4b71-4af1-b6cb-2864607c7794`. Automatic-distribution group **IGC Internal** contains build 1 and invited `henryberkin@gmail.com`. No external group, beta-review submission or App Review submission was created. Owner installation and smoke feedback remain the next validation observation. |
| Release candidate — Designer / QA | Capture truthful screenshots: iPhone 6.9-inch and iPad 13-inch because native iPad support is intended; 1–10 non-transparent JPEG/JPG/PNG images per device class. | **Passed 2026-07-31** | Seven dark-appearance screenshots per device generated from the running app by `AppStoreScreenshotTests` at 1320×2868 and 2064×2752, verified free of duplicates, and uploaded to the 6.9-inch and 13-inch slots. Assets show only shipped 1.0 states. [Screenshot specifications](https://developer.apple.com/help/app-store-connect/reference/app-information/screenshot-specifications/) |
| Release candidate — PM / App Store Reviewer | Finalise name/subtitle, description, keywords, category, age-rating questionnaire, copyright, primary language, support/privacy URLs, reviewer contact and notes. | **Passed 2026-07-31** | Name, subtitle, description, keywords, promotional text, category Finance, age rating 4+, copyright `2026 Henry Berkin`, primary language English (U.K.), support/privacy/marketing URLs and reviewer contact are entered and recorded in `docs/APP_STORE_LISTING.md`. The description's accessibility paragraph was corrected 2026-08-01 while Waiting for Review. |
| Record setup / before submission — Product Owner | Create the App Store Connect record after immutable SKU/bundle approval; choose territories and answer EU trader-status prompts as applicable before distribution. | **Record passed / distribution inputs open** | Record `6796327865` was created as approved. Territory is **United Kingdom only** (IGC-D029), price is free (IGC-D030), and the EU DSA trader declaration is therefore not applicable to this release; adding any EU territory later requires a publishable address first. [App information](https://developer.apple.com/help/app-store-connect/reference/app-information/app-information/) |
...
- Final App Privacy, age-rating, metadata, screenshots, reviewer contact/notes,
  copyright/category and territory/trader responses remain open.
- Internal TestFlight is available. External beta still requires final beta-review
  contact details (including owner phone number), external test-group decisions and
  any required Beta App Review. App Store submission remains separately unauthorised.
