...
| Bundle identifier and reverse-DNS namespace | **Open owner input** | Product Owner must select an identifier in the owning Team’s namespace before Xcode-project creation. Bundle ID must match the build and cannot be changed after upload. [App information](https://developer.apple.com/help/app-store-connect/reference/app-information/app-information/) |
| Apple Developer Team, signing ownership, Account Holder access | **Open owner input** | Confirm the legal/operational owner before project creation. Do not share individual credentials or invent a team. |
